...
void UKismetVisMeshLibrary::GenerateBoxMesh(FVector BoxRadius, TArray<FVector>& Vertices, TArray<int32>& Triangles,
	TArray<FVector>& Normals, TArray<FVector2D>& UVs, TArray<FVisMeshTangent>& Tangents)
{
	// Generate verts
	FVector BoxVerts[8];
...
void UKismetVisMeshLibrary::CalculateTangentsForMesh(const TArray<FVector>& Vertices, const TArray<int32>& Triangles,
	const TArray<FVector2D>& UVs, TArray<FVector>& Normals, TArray<FVisMeshTangent>& Tangents)
{
...
void UKismetVisMeshLibrary::CreateGridMeshWelded(int32 NumX, int32 NumY, TArray<int32>& Triangles,
	TArray<FVector>& Vertices, TArray<FVector2D>& UVs, float GridSpacing)
{
	Triangles.Empty();
	Vertices.Empty();
	UVs.Empty();
...
void UKismetVisMeshLibrary::CreateGridMeshSplit(int32 NumX, int32 NumY, TArray<int32>& Triangles,
	TArray<FVector>& Vertices, TArray<FVector2D>& UVs, TArray<FVector2D>& UV1s, float GridSpacing)
{
	Triangles.Empty();
	Vertices.Empty();
	UVs.Empty();
...
void UKismetVisMeshLibrary::GetSectionFromStaticMesh(UStaticMesh* InMesh, int32 LODIndex, int32 SectionIndex,
...
void UKismetVisMeshLibrary::CopyVisMeshFromStaticMeshComponent(UStaticMeshComponent* StaticMeshComponent,
	int32 LODIndex, UVisMeshComponent* ProcMeshComponent, bool bCreateCollision)
{
...
void UKismetVisMeshLibrary::GetSectionFromVisMesh(UVisMeshComponent* InProcMesh, int32 SectionIndex,
	TArray<FVector>& Vertices, TArray<int32>& Triangles, TArray<FVector>& Normals, TArray<FVector2D>& UVs,
	TArray<FVisMeshTangent>& Tangents)
{
...
void UKismetVisMeshLibrary::SliceVisMesh(UVisMeshComponent* InProcMesh, FVector PlanePosition,
	FVector PlaneNormal, bool bCreateOtherHalf, UVisMeshComponent*& OutOtherHalfProcMesh,
	EVisMeshSliceCapOption CapOption, UMaterialInterface* CapMaterial)
{
	if (InProcMesh != nullptr)
	{
		...
		if (bCreateOtherHalf)
		{
			...
			OutOtherHalfProcMesh = NewObject<UVisMeshComponent>(InProcMesh->GetOuter());
